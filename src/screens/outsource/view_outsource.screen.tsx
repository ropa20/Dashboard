...
import './view_outsource.screen.scss';
...
export default function ViewOutsource() {
  ...
  const GetOutsource = async () => {
    try {
      const response: any = await Models.outsource.getOutsource({
        outsource_id: id,
      });
  ...
  const inputFields: IAddValues[] = [
    { label: "Organization", key: "organization.name", type: "string", isNested: true },
  ...
  return (
    <div className="view_outsource_container">
      <div className="view_outsource_wrapper">
        <History name={state.data.name} />
        <div className="view_outsource_body_container">
          <View
            actions={[
              { link: `/edit_outsource/${id}`, icon: 'edit' },
